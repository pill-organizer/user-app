...
class SettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<SettingsBloc>(
      create: (context) =>
          SettingsBloc(repository: context.read<DeviceRepository>())..add(SettingsLoadRequested()),
      child: Scaffold(
                    ...
                    children: [
                      ListTile(
                        leading: Container(
                          ...
                          child: const Icon(Icons.pin_outlined, color: AppTheme.primaryColor),
                        ),
                        title: const Text('PIN Settings'),
                        subtitle: Text(
                          config?.pinHash != null ? 'PIN is configured' : 'No PIN set',
                        ),
                      ...
                      ListTile(
                        leading: Container(
                          ...
                          child: const Icon(Icons.nfc, color: AppTheme.secondaryColor),
                        ),
                        title: const Text('RFID Tokens'),
                        subtitle: Text('${config?.rfidTokens.length ?? 0} tokens configured'),
                        trailing: const Icon(Icons.chevron_right),
                        onTap: () {
                    ...
                    children: [
                      ListTile(
                        leading: Container(
                          ...
                          child: const Icon(Icons.thermostat, color: AppTheme.accentColor),
                        ),
                    ...
                    children: [
                      SwitchListTile(
                        secondary: Container(
                          ...
                          child: const Icon(Icons.volume_up, color: AppTheme.warningColor),
                        ),
                        title: const Text('Buzzer'),
                        subtitle: const Text('Device buzzer for pill reminders'),
                        value: config?.buzzerEnabled ?? true,
                        onChanged: (value) {
                          context.read<SettingsBloc>().add(SettingsToggleBuzzer(value));
                        },
                      ),
                      const Divider(height: 1),
                      SwitchListTile(
                        ...
                        onChanged: (value) {
                          context.read<SettingsBloc>().add(SettingsTogglePushNotifications(value));
                        },
                      ),
                    ],
                  ),
  ...
  void _showRfidTokensDialog(BuildContext context, List<String> tokenHashes) {
    showDialog(
      context: context,
                    ...
                    return ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () {
                          ...
                          context.read<SettingsBloc>().add(SettingsRemoveRfidToken(hash));
                        },
                      ),
                    );
                  },
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              // Global listener in app.dart handles navigation to login
            },

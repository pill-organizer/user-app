...
import '../../common/bloc/environmental_bloc.dart';
...
class EnvironmentalHistoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<EnvironmentalBloc>(
  ...
  Widget _buildChartContent({
    ...
    if (state.period == HistoryPeriod.day) {
      ...
      final spots = state.hourlyData.asMap().entries.map((entry) {
        return FlSpot(entry.key.toDouble(), getValueHourly(entry.value));
      }).toList();
      ...
      final spots = state.dailyData.asMap().entries.map((entry) {
        return FlSpot(entry.key.toDouble(), getAvgDaily(entry.value));
      }).toList();
